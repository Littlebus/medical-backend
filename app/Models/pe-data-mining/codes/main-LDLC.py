...
def decisionTree():
    ...
    clf = tree.DecisionTreeClassifier(criterion='entropy', min_samples_leaf=1, min_samples_split=2, max_depth=3)
    clf.fit(X_train, y_train)
    joblib.dump(clf, "models/LDLC.decisionTree.model")
...
def xgboost():
    X_train, X_test, y_train, y_test, X_labels = get_data(one_hot=False)
    xr = xgb.XGBClassifier(learning_rate=0.001, n_estimators=1000, n_jobs=8, min_child_weight=1, max_depth=3, gamma=0, colsample_bytree=0.6, subsample=0.8)
    xr.fit(X_train, y_train)
    joblib.dump(xr, "models/LDLC.xgboost.model")
...
def randomForest():
    X_train, X_test, y_train, y_test, X_labels = get_data(one_hot=False)
    rf = RandomForestClassifier(n_estimators=1000, criterion='gini', min_samples_leaf=2, min_samples_split=5, max_depth=None)
    rf.fit(X_train, y_train)
    joblib.dump(rf, "models/LDLC.randomForest.model")
