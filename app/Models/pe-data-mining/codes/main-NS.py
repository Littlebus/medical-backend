...
def decisionTree():
    ...
    joblib.dump(clf, "models/NS.decisionTree.model")
...
def xgboost():
    X_train, X_test, y_train, y_test, X_labels = get_data(one_hot=False)
    xr = xgb.XGBClassifier(learning_rate=0.001, n_estimators=1000, n_jobs=8, min_child_weight=4, max_depth=4, gamma=0, colsample_bytree=0.9, subsample=0.9)
    xr.fit(X_train, y_train)
    joblib.dump(xr, "models/NS.xgboost.model")
...
def randomForest():
    X_train, X_test, y_train, y_test, X_labels = get_data(one_hot=False)
    rf = RandomForestClassifier(n_estimators=1000, criterion='entropy', min_samples_leaf=3, min_samples_split=5, max_depth=None)
    rf.fit(X_train, y_train)
    joblib.dump(rf, "models/NS.randomForest.model")
