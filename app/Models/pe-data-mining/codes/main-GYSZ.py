...
def get_data(one_hot=False):
    ...
    # label = data.gen_label(all1, all2, "低密度脂蛋白", 2.1, label="Y")
    # label = data.gen_label(all1, all2, "尿酸", 420, label="Y")
    label = data.gen_label(all1, all2, "甘油三酯", 1.7, label="Y")
...
def decisionTree():
    ...
    clf = tree.DecisionTreeClassifier(criterion='gini', min_samples_leaf=2, min_samples_split=1.0, max_depth=3)
    clf.fit(X_train, y_train)
    joblib.dump(clf, "models/GYSZ.decisionTree.model")
...
def xgboost():
    X_train, X_test, y_train, y_test, X_labels = get_data(one_hot=False)
    xr = xgb.XGBClassifier(learning_rate=0.0001, n_estimators=1000, n_jobs=8, min_child_weight=1, max_depth=4, gamma=0, colsample_bytree=0.9, subsample=0.9)
    xr.fit(X_train, y_train)
    joblib.dump(xr, "models/GYSZ.xgboost.model")
...
def randomForest():
    X_train, X_test, y_train, y_test, X_labels = get_data(one_hot=False)
    rf = RandomForestClassifier(n_estimators=1000, criterion='entropy', min_samples_leaf=2, min_samples_split=5, max_depth=None)
    rf.fit(X_train, y_train)
    joblib.dump(rf, "models/GYSZ.randomForest.model")
